...
sender_address = 'votingvathiyar@gmail.com' #enter sender's email id
sender_pass = 'nyarmjchbfvnhgkv' #enter sender's password
...
def voting():
    if request.method=='POST':
        pkl_file = open(r'C:\xampp\htdocs\Smart-Voting-System-main\code\encoder.pkl', 'rb')
        my_le = pickle.load(pkl_file)
        pkl_file.close()
        recognizer = cv2.face.LBPHFaceRecognizer_create() # cv2.createLBPHFaceRecognizer()
        recognizer.read(r"C:\xampp\htdocs\Smart-Voting-System-main\code\Trained.yml")
        cam=cv2.VideoCapture(0, cv2.CAP_DSHOW)
